...
in the list of winning numbers. The first match makes the card worth one point and each match after the first doubles the point value of that card.
````
For example:
 ...
 */
fn first_part() {
    let file_content: Vec<String> = commons::get_input(INPUT_FILE_NAME);
    let mut output = 0;

    for line in file_content {
        let card_numbers: Vec<&str> = line.split(":")
            .collect::<Vec<&str>>()
            .get(1)
            .expect("Card should have winning and owned numbers")
            .trim()
            .split("|")
            .collect::<Vec<&str>>();

        let winning_numbers: Vec<&str> = card_numbers.get(0)
            .expect("Card should have winning numbers")
            .split_whitespace()
            .collect::<Vec<&str>>();

        output += card_numbers.get(1)
            .expect("Card should have owned numbers")
            .split_whitespace()
            .filter(|wn| winning_numbers.contains(wn))
            .map(|x| x.parse::<i32>().expect("All values should be numbers"))
            .enumerate()
            .fold(0, |acc, (i, _)| {
                if i == 0 { acc + 1 } else { acc * 2 }
            });
    }
    println!("{output}"); // 23678
}
